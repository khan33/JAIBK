...
class SearchByVIN: UIViewController {
...
//        view.alignment = .fill
    ...
    private (set) lazy var lblChosing: UILabel = { [unowned self] in
        ...
        label.text = "Search by Vehicle VIN number"
        label.textAlignment = .left
        label.font = UIFont(name: AppFontName.book, size: 14)
        return label
    }()
...
extension SearchByVIN {
   func configNav() {
    ...
    func setupView() {
        ...
        let VINNumberView = TextInputView(placeholder: "VIN Number", isPasswordEnable: true, icon: "") { [weak self] (enteredText) in
